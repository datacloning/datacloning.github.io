...
mu <- 2.5
sigma_sq <- 0.2^2
eps <- rnorm(n * 2, mean = 0, sd = sqrt(sigma_sq))
tau_sq <- 0.5^2
alpha <- rep(rnorm(n, mean = 0, sd = sqrt(tau_sq)), 2)
Y <- mu + alpha + eps
dim(Y) <- c(n, 2)
summary(Y)
```
...
dat <- list(Y = dcdim(array(Y, c(n, 2, 1))), n = n, K = 1)
str(dat)
dcfit1 <- dc.fit(data = dat, 
    params = c("mu", "sigma_sq", "tau_sq"), 
    model = model, n.iter = 1000,
    n.clones = c(1, 10, 25, 50),
    unchanged = "n", multiply = "K")
...
$i=1, 2, \ldots, n$ observations, $j=1, 2, \ldots, m$ clusters of obs, 2 obs per cluster,
$$logit(p_{ij}) = \beta_{0} + \beta_{1} X_{ij} + \alpha_{j}$$,
$$\alpha_{j} \sim Normal(0, \sigma^2)$$.
...
The second type of asymptotics does not lead to consistent MLE of $\sigma^2$. 
That was precisely the point in Kiefer and Wolfowitz[^KW].

[^KW]: Kiefer, J. and Wolfowitz, J., 1956. 
  Consistency of the maximum likelihood estimator in the presence of infinitely many incidental parameters. *Ann. Math. Statist.*, **27**, 887&ndash;906. [PDF from Project Euclid](http://projecteuclid.org/euclid.aoms/1177728066)
...
corresponding to $\alpha_{j}$. In this asymptotics, the information about $\sigma^2$ 
...
```r
glmm.model.mlogit2 <- function(){
    for (k in 1:ncl) {
        for (i in 1:I){
            alpha[i,k] ~ dnorm(0, tau)
            for (j in 1:m){
                Y[j,i,k] ~ dbin(p[j,i,k], 1)
                logit(p[j,i,k]) <- alpha[i,k] + beta[1] + beta[2] * X[j,i]
            }
        }
    }
    for (j in 1:np){
...
### DC for replicates within clusters

```r
glmm.model.mlogit3 <- function(){
    for (i in 1:I){
        alpha[i] ~ dnorm(0, tau)
        for (j in 1:m){
            Y[j,i] ~ dbin(p[j,i], 1)
...
Y2 <- dcdim(array(Y, c(m, I, 1)))
x2 <- matrix(x, m, I)
dat2 <- list(Y=Y2, X=x2, I=I, m=m, np=2, ncl=1)

Y3 <- matrix(Y, m, I)
x3 <- matrix(x, m, I)
dat3 <- list(Y=Y3, X=x3, I=I, m=m, np=2)

cl <- makeSOCKcluster(6)
ncl <- c(1,2,5,10)
mod2 <- dc.parfit(cl, dat2, c("beta", "sigma2"), glmm.model.mlogit2, 
    n.clones = ncl, multiply="ncl", unchanged=c("I", "m", "np", "X"), n.update=2000, n.iter=1000)
mod3 <- dc.parfit(cl, dat3, c("beta", "sigma2"), glmm.model.mlogit3, 
    n.clones = ncl, multiply="m", unchanged=c("I", "np"), n.update=2000, n.iter=1000)
stopCluster(cl)
...
library(lme4)
mod.lm <- lmer(Y ~ x + (1|g), family = binomial(link=logit), nAGQ=60,
doFit=TRUE)
summary(mod.lm)

cbind(c(beta, sigma2=sigma^2), est=coef(mod2), se=dcsd(mod2))
```
