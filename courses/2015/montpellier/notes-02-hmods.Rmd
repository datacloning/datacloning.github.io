...
* $$p_i = \frac{exp(\theta_0 + \theta_1 x_i)}{1 + exp(\theta_0 + \theta_1 x_i)}$$.
...
* Observation model: $$ (Y_i \mid N_i) \sim Binomial(N_i, p) $$ for visits $t=1,2, \ldots, T$.
...
L_mat <- matrix(grid$L, sqrt(nrow(grid)))
image(L_mat, 
    ylab = "p", xlab = expression(varphi),
    col = dcpal_reds(12))
abline(v = phi, h = p, col = "#f9f2f4", lwd = 3)
abline(v = phi, h = p, col = "#c7254e", lwd = 1)
curve((p * phi) / x, 0, 1, add = TRUE, 
    col = "#18bc9c", lwd = 2)
```
