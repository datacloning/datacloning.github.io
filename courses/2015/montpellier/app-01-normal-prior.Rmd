...
```{r, echo=FALSE}
## TODO
## - implement logit scaling (normal prior)
## - 2 panes showing 2 scales? What ranges?
inputPanel(
...
renderPlot({
    ...
    fPri <- function(p, mu, sig2) {
        dnorm(qlogis(p), mu, sqrt(sig2))
    }

    Lik <- sapply(pval, fLik, y=y[1:input$n])
    Pri <- sapply(pval, fPri, input$mu, input$sig2)
    Pos <- Lik * Pri
